...
void FixedWing::calcLonDyn(double pVa2S_2,double de,dyn::Wrench& f_tau)
{
    ...
    double C_D{m_p.C_D_p + (m_p.C_L_0+m_p.C_L_alpha*m_x.alpha)*(m_p.C_L_0+m_p.C_L_alpha*m_x.alpha)/
                (3.14159*m_p.AR*m_p.e)};

    double lift = pVa2S_2*(C_L+m_p.C_L_q*m_p.c*m_states.w(0)/2.0/m_x.Va + m_p.C_L_de*de);
    double drag = pVa2S_2*(C_D+m_p.C_D_q*m_p.c*m_states.w(0)/2.0/m_x.Va+m_p.C_D_de*de);
    double m{pVa2S_2*m_p.c*(m_p.C_m_0+m_p.C_m_alpha*m_x.alpha+m_p.C_m_q*m_p.c*m_states.w(2)/2.0/m_x.Va+m_p.C_m_de*de)};
...
{
    double fy{pVa2S_2*(m_p.C_Y_0+m_p.C_Y_beta*m_x.beta+m_p.C_Y_p*m_p.b/2.0/m_x.Va*m_states.w(0)+
              m_p.C_Y_r*m_p.b/2.0/m_x.Va*m_states.w(2)+m_p.C_Y_da*da+m_p.C_Y_dr*dr)};
    double l{pVa2S_2*m_p.b*(m_p.C_l_0+m_p.C_l_beta*m_x.beta+m_p.C_l_p*m_p.b/2.0/m_x.Va*m_states.w(0)+
             m_p.C_l_r*m_p.b/2.0/m_x.Va*m_states.w(2)+m_p.C_l_da*da+m_p.C_l_dr*dr)};
    double n{pVa2S_2*m_p.b*(m_p.C_n_0+m_p.C_n_beta*m_x.beta+m_p.C_n_p*m_p.b/2.0/m_x.Va*m_states.w(0)+
             m_p.C_n_r*m_p.b/2.0/m_x.Va*m_states.w(2)+m_p.C_n_da*da+m_p.C_n_dr*dr)};
...
void FixedWing::calcMotorDyn(double dt,dyn::Wrench &f_tau)
{
    ...
    double a{m_p.rho*std::pow(m_p.D_prop,5)/(4*6.28318)*m_p.C_Q0};
    double b{m_p.rho*std::pow(m_p.D_prop,4)/(2*3.14159)*m_p.C_Q1*m_x.Va+m_p.KQ*m_p.K_V/m_p.R_motor};
